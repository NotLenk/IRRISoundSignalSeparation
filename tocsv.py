import os
import pandas as pd
from sklearn.model_selection import train_test_split # am gasit pe net ca se foloseste
...
def createDatasets():
    ...
    # Save to CSV files
    speech_train.to_csv('Speech_Train.csv', index=False)
    speech_valid.to_csv('Speech_Valid.csv', index=False)
    speech_test.to_csv('Speech_Test.csv', index=False)

    noise_train.to_csv('Noise_Train.csv', index=False)
    noise_valid.to_csv('Noise_Valid.csv', index=False)
    noise_test.to_csv('Noise_Test.csv', index=False)
