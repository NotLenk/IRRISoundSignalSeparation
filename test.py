...
def preprocess_audio(file_path, n_fft=1024, hop_length=512):
    ...
    magnitude_db_slices = [magnitude_db[:, i:i+513] for i in range(0, magnitude_db.shape[1], 513)]
...
def predict(model, magnitude_db_slices, device='cpu'):
    ...
    for slice in magnitude_db_slices:
        if slice.shape[1] < 513:
            padding = np.zeros((slice.shape[0], 513 - slice.shape[1]))
